...
private:
    ...
    std::vector<std::vector<node>> XYVec;

    std::vector<maze::node> rand_Dir(const node pos);
    void Wall_breaker(const maze::node pos, const maze::node pos2);
    friend std::ostream &operator<<(std::ostream &COUT, const maze::node &pos);
};
